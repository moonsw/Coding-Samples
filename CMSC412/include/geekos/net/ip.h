...
extern int IP_Transmit(IP_Address * src, IP_Address * destination, struct Net_Buf *, uchar_t tos, ushort_t protocol);
...
extern int IP_Device_Configure(char *name, IP_Address * ipAddress, Netmask * netmask);
extern int IP_Device_Stat(struct IP_Device_Info *info, ulong_t deviceCount, char *name);
extern int IP_Dispatch_Table_Add(ushort_t type, int (*dispatcher) (struct IP_Device *, IP_Address *, IP_Address *, struct Net_Buf *));
