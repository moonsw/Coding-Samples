...
struct Kernel_Thread {
  ...
  int refCount;
  int detached;               /* detached process don't linger on Exit as zombies waiting for Wait to reap. */
  ...
  signal_handler signalHandler[MAXSIG];
  bool pendingSignal[MAXSIG];
  bool busySignal;
  void *sigReturnAddress;
  char threadName[20];
...
extern struct Kernel_Thread *Return_Zombie_Thread(void);
